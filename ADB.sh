...
if [ $? -ne 0 ]; then
echo -e "${CYAN}Menghubungkan ke $LAST_IP:$PORT...${NC}"
adb connect $LAST_IP:$PORT
sleep 1
else
echo -e "${GREEN}Sudah terhubung ke $LAST_IP${NC}"
fi
sleep 1
}
...
autodetect_usb() {
clear
echo -e "${CYAN}Colok HP via USB dulu...${NC}"
adb devices
read -p "Tekan Enter jika sudah..."

IP=$(adb shell ip route | grep -oE 'src [0-9.]+' | awk '{print $2}')

if [ -z "$IP" ]; then
echo -e "${RED}Gagal mendeteksi IP!${NC}"
pause
return
fi

echo -e "${GREEN}IP ditemukan: $IP${NC}"
...
while true; do
clear
echo -e "${GREEN}Path: $currentpath${NC}"
echo "ls | cd | pull | back"
read -p ">> " cmd
...
4) explorer ;;
5) adb shell ;;
6) adb shell sh /storage/emulated/0/Android/data/moe.shizuku.privileged.api/start.sh
   pause
   ;;
7) read -p "Package: " pkg
...
10) read -p "Nama: " nama
    ...
    pause
    ;;
11) echo "[1] Reboot [2] Recovery [3] Bootloader"
    read -p "Pilih: " r
